...
const FastFoods = () => {
    ...
    const fastFoodMenus = menu_list.find(item => item.category === "Fast Food")?.menus || [];
    ...
    const nextSlide = () => {
        setCurrentSlide((prev) => 
            prev === fastFoodMenus.length - 1 ? 0 : prev + 1
        );
    };

    const prevSlide = () => {
        setCurrentSlide((prev) => 
            prev === 0 ? fastFoodMenus.length - 1 : prev - 1
        );
    };
    ...
    return (
        <>
            <div className="traditional-section container mt-5 pt-5">
                <h2 className="text-center mb-4">Fast Food Selection</h2>
